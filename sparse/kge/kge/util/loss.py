...
class RankingLoss(KgeLoss):
    def __init__(self, config, **kwargs):
        ...
        self.use_noise = self.config.get("diff_rank.use_noise")
        if self.use_noise:
            Ranker.use_noise = True
            Ranker.noise_k = self.config.get("diff_rank.k")
            Ranker.noise_tau = self.config.get("diff_rank.tau")
        Ranker.scale_gradient = self.config.get("diff_rank.scale_gradient")

        self._lambda = config.get("diff_rank.lambda")

    def __call__(self, scores, labels, **kwargs):
        labels = self._labels_as_indexes(scores, labels)
        _lambda = self.config.get("train.loss_arg")
        ranks = self._ranker(scores, Variable(torch.tensor(self._lambda)))
        final_ranks = ranks[torch.arange(labels.size(0)), labels]
        #rank_loss = - 1 / torch.sum(final_ranks)
...
class Ranker(torch.autograd.Function):
    ...
    def forward(ctx, input, _lambda):
        ...
        """
        if Ranker.use_noise:
            samples = Ranker.sample_gumbel(input.shape, k=Ranker.noise_k, tau=Ranker.noise_tau).to(input.device)
            ctx.samples = samples
            input = input + samples.to()
    ...
    def backward(ctx, grad_output):
        ...
        if Ranker.use_noise:
            perturbed_input = perturbed_input + ctx.samples
        perturbed_arg_ranks = torch.argsort(
            torch.argsort(perturbed_input, dim=1, descending=True)
        ) + 1

        pos = (1 / (arg_ranks[grad_output > 0] - 0.99))  # 0.99 prevents divide by 0 error, note that in these cases the
                                                         # gradients are 0 anyway, so it does not have effect but it
                                                         # prevents the parameter to become .nan
        if not Ranker.scale_gradient:
            pos = 1
        return (- 1/_lambda * (arg_ranks - perturbed_arg_ranks)) * pos, None

    @staticmethod
    def sample_gumbel(shape, k, tau):
        num_gamma = 10.0
        gammas = torch.zeros(shape)
        for i in range(1, int(num_gamma) + 1):
            gamma = torch.distributions.gamma.Gamma(1 / k, k / i)
            gammas += gamma.sample(shape)
        gammas -= torch.log(torch.tensor(num_gamma))
        samples = tau * (gammas / k)
        return samples
