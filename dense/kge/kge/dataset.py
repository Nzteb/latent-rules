...
"""
    ...
    def load_rule_meta(self):

        if self.loaded_rule_meta:
            pass

        meta_folder = self.config.get("neural_aggregator.logic_input.folder")
        folder = os.path.join(self.folder, meta_folder)

        rule_map_f = os.path.join(folder, "rule_map.pkl")
        with open(rule_map_f, 'rb') as f:
            self._rule_map = pickle.load(f)

        rule_feature_f = os.path.join(folder, "rule_features.pkl")
        with open(rule_feature_f, 'rb') as f:
            self._rule_features = pickle.load(f)

        self.rule_conf = torch.zeros(len(self._rule_features) + 1)
        for k, v in self._rule_features.items():
            conf = float(v[1]) / (float(v[0]) + 5)
            self.rule_conf[k] = conf

        candidates_sp_f = os.path.join(folder, "processed_sp_train.pkl")
        with open(candidates_sp_f, 'rb') as f:
            self._cand_sp_train = pickle.load(f)

        candidates_po_f = os.path.join(folder, "processed_po_train.pkl")
        with open(candidates_po_f, 'rb') as f:
            self._cand_po_train = pickle.load(f)

        if not self.config.get("neural_aggregator.run_test"):
            candidates_sp_f = os.path.join(folder, "processed_sp_valid.pkl")
            with open(candidates_sp_f, 'rb') as f:
                self._cand_sp_valid = pickle.load(f)

            candidates_po_f = os.path.join(folder, "processed_po_valid.pkl")
            with open(candidates_po_f, 'rb') as f:
                self._cand_po_valid = pickle.load(f)
        else:
            candidates_sp_f = os.path.join(folder, "processed_sp_test.pkl")
            with open(candidates_sp_f, 'rb') as f:
                self._cand_sp_valid = pickle.load(f)

            candidates_po_f = os.path.join(folder, "processed_po_test.pkl")
            with open(candidates_po_f, 'rb') as f:
                self._cand_po_valid = pickle.load(f)
        self.loaded_rule_meta = True
