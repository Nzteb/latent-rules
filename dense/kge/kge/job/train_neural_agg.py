...
class TrainingNeuralAggregation(TrainingJob):
    ...
    def _get_collate(self):
        ...
        def collate(batch):
            ...
            max_num_rules = self.model.get_option("encoder.max_num_rules")
    ...
    def _prepare(self):
        ...
        rule_feat = self.dataset._rule_features
        scorer = self.model.get_scorer()
        scorer.rule_conf = torch.zeros(len(rule_feat) + 1)
        for k, v in rule_feat.items():
            conf = float(v[1]) / (float(v[0]) + 5)
            scorer.rule_conf[k] = conf
    ...
    def _process_subbatch(
        ...
        scoring_type = self.model.get_option("scoring_type")
        scoring_type = "ce"
        ...
        for idx, indices_sp in enumerate(sp_input):
            ...
            if scoring_type == "noisy_or":
                true_idx = sp_labels[idx]
                labels = torch.zeros(scores_sp.shape, device=self.device, dtype=torch.float)
                labels[true_idx] = 1.0
                loss_value_sp = loss(scores_sp, labels)
            else:
        ...
        for idx, indices_po in enumerate(po_input):
            ...
            if scoring_type == "noisy_or":
                true_idx = po_labels[idx]
                labels = torch.zeros(scores_po.shape, device=self.device, dtype=torch.float)
                labels[true_idx] = 1.0
                loss_value_po = loss(scores_po, labels)
            else:
