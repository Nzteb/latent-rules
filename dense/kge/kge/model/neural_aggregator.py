...
class NeuralAggregatorScorer(RelationalScorer):
    ...
    def __init__(self, config: Config, dataset: Dataset, configuration_key=None):
        ...
        self.pos_encoder = PositionalEncoding(d_model=self.emb_dim, dropout=dropout)
        self.num_embeddings = len(self.dataset._rule_features)
        # last index holds the padding token embedding
        self.embeddings = torch.nn.Embedding(self.num_embeddings+1, emb_dim, sparse=True)
    ...
    def score_indices(self, indices):
        ...
        if self.get_option("positional"):
            emb *= math.sqrt(self.emb_dim)
            emb = self.pos_encoder(emb)

        # 1 x batch_size x emb_dim
        repeat = self.cls_embedding.weight.repeat((indices.size()[1], 1)).unsqueeze(0)

        # (max_num_rules +1) x batch_size x emb_dim
        cat = torch.cat((repeat, emb))

        # batch_size x (max_num_rules +1)
        # mask is 1 for indices to ignore
        padd_mask = torch.zeros(indices.size()[1], indices.size()[0]+1).to(self.embeddings.weight.device)

        # first word is cls embedding
        padd_mask[:, 1:] = (indices == self.num_embeddings).permute(1, 0)
        padd_mask = padd_mask.to(torch.bool)

        scores = self.encoder_score_emb(cat, padd_mask, confs)
        return scores

    def encoder_score_emb(self, emb, padding_mask, confs):
        """
        ...
        aob = self.get_option("attend_only_backward")
        if aob:
            src_mask = torch.triu(torch.ones((emb.shape[0], emb.shape[0]), device=self.embeddings.weight.device), diagonal=1)
        else:
            src_mask = None
        if not (scoring_type in ["no_transform", "shallow_sum", "shallow_noisy"]):
            # max_num_rules x batch_size x emb_dim
            transform = self.encoder.forward(emb, src_key_padding_mask=padding_mask, mask=src_mask.to(torch.bool))
        else:
            transform = emb

        if scoring_type == "cls":  # score with cls token
            return self.final_scorer.forward(transform[0])
        elif scoring_type in ["pool", "no_transform", "noisy_or", "shallow_sum", "shallow_noisy"]:
            # cut of cls token
            transform = transform[1:]
            # use pooling
            # when pooling is used, the padding output embeddings must be discarded
            ...
            if scoring_type in ["shallow_noisy", "shallow_sum"]:
                ...
                prob = max_transform
                #prob = torch.sigmoid(max_transform)
    ...
    def score_emb(self, s_emb, p_emb, o_emb, combine: str):
        ...
        if combine == "o":
            ...
            for i in range(len(s_emb)):
                ...
                out[i] = self.score_indices(
                    torch.tensor(rules)[:self.get_option("encoder.max_num_rules")].view(-1, 1).to(out.device)
                )
            return out
        ...
        # combine is "spo" and direction is "s"
            ...
            max_num_rules = self.get_option("encoder.max_num_rules")
            meta_sp = self.meta_sp
